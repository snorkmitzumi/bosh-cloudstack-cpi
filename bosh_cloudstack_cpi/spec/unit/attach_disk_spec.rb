...
describe Bosh::CloudStackCloud::Cloud do
  context "when volume is detached from server" do
    ...
    it "attaches a CloudStack volume to a server" do
      job = generate_job
  ...
  context "when volume is already attached to server" do
    let(:server) { double("server", :id => "i-test", :name => "i-test" ) }
    let(:volume) { double("volume", :id => "v-foobar", :server_id => "i-test", :device_id => 2 ) }
    let(:cloud)  {
      mock_cloud do |compute|
        compute.servers.should_receive(:get).with("i-test").and_return(server)
        compute.stub(:volumes).and_return([volume])
    ...
    it "attaches an attached CloudStack volume to a server" do
      old_settings = {
        "foo" => "bar",
        "disks" => {
          "persistent" => {
